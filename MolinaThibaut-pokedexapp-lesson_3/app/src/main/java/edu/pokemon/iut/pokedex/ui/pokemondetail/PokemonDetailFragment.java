...
public class PokemonDetailFragment extends BaseFragment implements PokemonGestureListener.Listener {
    ...
    protected  ImageView pokemonCaptureView;
    ...
    private void initView(Pokemon pokemon, PokemonViewModel viewModel) {
        if(pokemon != null) {
            ...
            if(pokemon.isCapture()){
                Glide.with( getContext() ).load( R.drawable.ic_launcher_pokeball ).into( pokemonCaptureView);
            }
            //TODO 30) RAJOUTER UN LISTENER SUR L'IMAGE DE LA POKEBALL EST ACTIVER LA CAPTURE DU POKEMON AVEC LE viewModel
            pokemonCaptureView.setOnClickListener( new View.OnClickListener() {
                @Override
